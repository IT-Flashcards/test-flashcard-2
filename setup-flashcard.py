...
def open_vscode_and_run_ng(directory):
    ...
    npm_process = subprocess.Popen(["cmd", "/K", "cd /d " + directory + " && npm i"], creationflags=subprocess.CREATE_NEW_CONSOLE)
    npm_process.wait()  # Czekaj na zakończenie instalacji pakietów npm

    # Po zakończeniu npm i, uruchamianie Angulara
    ng_process = subprocess.Popen(["cmd", "/K", "cd /d " + directory + " && ng s -o"], creationflags=subprocess.CREATE_NEW_CONSOLE)
